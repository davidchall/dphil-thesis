...
three pixel layers and the first SCT layer. The seeds are then extended and fitted 
into the SCT and the TRT whilst resolving ambiguities and applying 
...
\item[Reconstruction] \hfill \\
	Muons are found by matching MS tracks to ID tracks\footnote{
		...
		ID tracks, \textit{segment-tagged} muons have ID tracks that match to 
...
\todo[inline]{Subthreshold muon overlap with electron => veto event}
