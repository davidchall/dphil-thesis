...
Although the signal acceptance in the \emch/\mech channels is maximised by using a looser 
\corrtrackmet cut in the 1-jet pre-selection, this does yield a sizeable dijet 
background, which has a large uncertainty. For this reason, two single-lepton transverse 
mass variables are constructed
\begin{equation}
...
VBF analysis (see \Figure~\todo{VBF BDT plot}).
...
\twojet bin a simple \mt fit is used. For the \eech/\mmch channels in the 0-jet and 1-jet 
bins, the \mt and \mll distributions are fit simultaneously. For the \emch/\mech channels 
in the 0-jet and 1-jet bins, a three dimensional fit of \mt, \mll and \ptsubleadlep is 
performed. The various fits enable maximal sensitivity to be extracted from the data.
