...
The uncertainties are parametrised by fitting the ratio of the \mt shapes, and then 
...
In cases when multiple variations exist within a single uncertainty source (such as the 
seven scale variations), the largest deviation from the nominal result is fit. A linear 
fit is used in the central \mt region, and a constant is used for the low-\mt and 
high-\mt tails of the distribution where statistical uncertainties dominate.
...
as a nuisance parameter in the \HWW fitting procedure. 
...
In the \twojet bin the \WW background is modelled by \sherpa, since the second jet is 
described by the parton shower in \meps{\powhegbox}{\pythia{6}}. Two kinds of diagrams 
...
The two production types are simulated separately
