...
\begin{figure}[t]
	...
	green blobs). The underlying event comprises multiple partonic interactions (purple 
	blob) and beam remnants (blue blobs). Photons (yellow) are also radiated.}
	\label{fig:mcevent}
\end{figure}
...
transition from the emissions of \iac{ME} to those of the parton shower. Second, each 
...
\begin{description}
\item[\mcatnlo] \hfill \\
	Simply adding a parton shower to \iac{NLO} \ac{ME} introduces double counting of 
	...
	\pythia{6} and \pythia{8} \cite{MCatNLO-Pythia}. A variant is also included in 
	\sherpa.
\item[POWHEG] \hfill \\
