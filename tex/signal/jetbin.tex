...
\braces{\sigma_0, \sigma_{\geq1}} basis, the covariance matrix is decomposed into two 
uncertainty sources
\begin{equation}
...
$\theta$ with uncertainty amplitudes $\nu(\Delta\theta)$ in the \braces{\sigma_0, 
\sigma_{\geq1}} basis:
\begin{equation}
	\begin{array}{l@{}l@{}l}
		\begin{array}{r}
...
$\nu(\theta)$ in the \braces{\sigma_0, \sigma_1, \sigma_{\geq2}} basis:
\begin{equation}
	\begin{array}{l@{}l@{}l}
		\begin{array}{r}
			\theta^{\text{yield}}              : \Big( \\
...
calculations to a variety of different \meps{\powhegbox}{\pythia{8}} configurations.
...
\begin{table}[t]
	\begin{tabular}{l@{\hskip 0.3in}r@{$\,\pm\,$}l@{\hskip 0.3in}c}
		\toprule
		& \multicolumn{2}{c@{\hskip 0.3in}}{$x$} & $\Delta x/x$ \\
		\midrule
...
\begin{figure}[t]
	...
	\todo[inline]{Parametrise $\epsilon_1$ result}}
...
it predicts $\sigma_{\total}$ to be \about10\% larger than that presented in 
